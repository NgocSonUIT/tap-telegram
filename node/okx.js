...
class OKX {
  ...
  async main() {
    const dataFile = path.join(__dirname, '../query/okx.txt')
    const userData = fs
    ...
    while (true) {
      for (let i = 0; i < userData.length; i++) {
        ...
        try {
          console.log(
            `========== Tài khoản ${i + 1} | ${extUserName} ==========`.blue
          )
          ...
          for (let j = 0; j < 16; j++) {
            ...
            const predict = Math.random() < 0.5 ? 0 : 1
            ...
            this.log(
              `Kết quả: ${result} x ${assessData.multiplier}! Balance: ${assessData.balancePoints}, Nhận được: ${calculatedValue}, Giá cũ: ${assessData.prevPrice}, Giá hiện tại: ${assessData.currentPrice}`
                .magenta
            )
