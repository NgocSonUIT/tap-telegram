...
class Dotcoin {
  ...
  loadCredentials() {
    try {
      const credentialsList = fs
        .readFileSync('../query/dotcoin.txt', 'utf8')
        .split('\n')
      return credentialsList.map((cred) => cred.trim())
    } catch (error) {
      console.error(
        "Không tìm thấy file 'authorization.txt'. Đảm bảo file nằm trong cùng thư mục với nhau."
      )
      return []
    }
  }

  async fetchTaskIds(authorization) {
    const url = 'https://api.dotcoin.bot/rest/v1/rpc/get_filtered_tasks'
    const headers = {
      ...this.getHeaders(authorization)
    }
    const data = { platform: 'ios', locale: 'en', is_premium: false }
    try {
      const response = await this.http(url, headers, data)
      if (response.status === 200) {
        const tasks = response.data
        return tasks.map((task) => task.id)
      } else {
        console.error(
          `Không tìm được nhiệm vụ, status code: ${response.status}`
        )
        return []
      }
    } catch (error) {
  ...
  async addAttempts(lvl, authorization, currentLevel) {
    const url = 'https://api.dotcoin.bot/rest/v1/rpc/add_attempts'
    const headers = {
      ...this.getHeaders(authorization)
    }
  ...
  async autoClearTask(authorization) {
    const taskIds = await this.fetchTaskIds(authorization)
    const url = 'https://api.dotcoin.bot/rest/v1/rpc/complete_task'
    const headers = {
      ...this.getHeaders(authorization)
    }
    for (const taskId of taskIds) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          console.log(
            `[ Task ${taskId} ] : Không thành công với mã trạng thái ${response.status}`
          )
        }
      } catch (error) {
  ...
  async saveCoins(coins, authorization) {
  ...
  async getUserInfo(authorization) {
    const url = 'https://api.dotcoin.bot/rest/v1/rpc/get_user_info'
    const headers = {
      ...this.getHeaders(authorization)
    }
    const data = {}
  ...
  async autoGame(authorization, coins) {
    const url = 'https://api.dotcoin.bot/rest/v1/rpc/try_your_luck'
    const headers = {
      ...this.getHeaders(authorization)
    }
    const data = { coins: coins }
  ...
  async upgradeDTCMiner(authorization) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.error(
          `[ DTC Miner ] : Lỗi khi nâng cấp với mã trạng thái ${response.status}`
        )
      }
    } catch (error) {
  ...
  async main() {
    ...
    while (true) {
      for (let index = 0; index < credentials.length; index++) {
        ...
        const info = await this.getUserInfo(authorization)
        console.log(
          `============== [ Tài khoản ${index} | ${info.first_name} ]`
        )

        if (!upgradeSuccess[authorization] && nUpgrade > 0) {
          for (let i = 0; i < nUpgrade; i++) {
            const currentLevel = info.daily_attempts
            const success = await this.addAttempts(
              0,
              authorization,
              currentLevel
            )
        ...
        if (info) {
          if (clearTask === 'y') {
            await this.autoClearTask(authorization)
          }
          ...
          await this.upgradeDTCMiner(authorization)
          await this.autoGame(authorization, 150000)
          const energy = info.daily_attempts
          if (energy > 0) {
            for (let i = 0; i < energy; i++) {
              process.stdout.write(`[ Tap ] : Tapping...`)
              await this.sleep(3000)
              await this.saveCoins(20000, authorization)
