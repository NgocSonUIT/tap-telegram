...
class Layernet {
  constructor() {
    this.dataFilePath = path.join(__dirname, '../query/layernet.txt')
  ...
  async getQueryIdAndUserData() {
    try {
      const data = fs
        .readFileSync(this.dataFilePath, 'utf8')
        .replace(/\r/g, '')
        .split('\n')
        .filter(Boolean)
  ...
  processGameData(data) {
    ...
    if (userRank && claimCountdown) {
      ...
      this.log(
        `Role: ${role}, ProfitPerHour: ${profitPerHour}, Balance: ${gold}, DOGS: ${dogs}`
      )
  ...
  async playRound(roundNumber) {
    ...
    const interval = setInterval(() => {
      if (messageCount < 60) {
        const inGameMessage = JSON.stringify([
          'inGame',
          { round: roundNumber, time: Date.now(), gameover: false }
        ])
    ...
    while (true) {
      try {
        ...
        for (let index = 0; index < data.length; index++) {
          ...
          const accessToken = await layernet.getAccessToken(
            queryIdLine,
            userData
          )
