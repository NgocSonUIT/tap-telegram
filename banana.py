...
def process_account(tg_info, perform_lottery, perform_quests):
    ...
    if token:
        ...
        print(f"{Fore.GREEN}====== Successfully Obtaining Data ======")
        time.sleep(1)  # Sleep for 1 second
        ...
        if perform_lottery or True:  # Always check lottery info
            ...
            print(f"{Fore.YELLOW}Remaining Time to claim Banana: "
            ...
            if remain_lottery_count > 0 and perform_lottery:
                ...
                if do_lottery_response.status_code == 200:
                    ...
                    print(f"{Fore.GREEN}====== Successfully Get Banana {banana_name} ======")
            ...
            if remaining_time_minutes <= 0:
                ...
                print(f"{Fore.YELLOW} Claiming Lottery {claim_lottery_response}...")

        # Perform quests if user chose to
        if perform_quests:
            print(f"{Fore.GREEN}====== Successfully Obtained Quest List ======")
...
# Claim quests if achieved and not claimed
        ...
        print(f"{Fore.GREEN}====== Processing Next Account.... ======")
...
def main():
    ...
    with open('query/banana.txt', 'r') as file:
    ...
    while True:
        ...
        for index, tg_info in enumerate(tg_infos, start=1):
            ...
            if tg_info:
                print(f"{Fore.CYAN}====== Processing Accounts {index} ======")
                token = login(tg_info)
                if token:
                    headers['Authorization'] = token
                    headers.update({
        ...
        #     print(f"{Fore.GREEN} Waiting {remaining_hours} hours {remaining_minutes} minutes {remaining_seconds} seconds before reprocessing...")
        #     time.sleep(min_remaining_time * 60)  # Sleep for the minimum remaining time before re-evaluating
        # else:
        #     print(f"{Fore.RED}There are no accounts that meet the claim criteria in the near future.")
        #     print(f"{Fore.GREEN}Waiting time before reprocessing...")
