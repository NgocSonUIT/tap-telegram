        ...
        if recover_coin_pool(token):
            ...
            if collect_info:
                ...
                if coin_pool_left_count > 0:
                    ...
                    if collect_result and collect_result.get('code') == 0:
                        collected_amount = collect_result['data']['collectAmount']
                        print(f"{Fore.GREEN + Style.BRIGHT}\r[ Collect ] : Đã thu thập thành công  {collected_amount} koin", flush=True)
                    else:
...
def main():
    ...
    while True:
        with open('query/yescoin.txt', 'r') as file:
        ...
        print(f"\n{random.choice(available_colors)+Style.BRIGHT}========={Fore.WHITE+Style.BRIGHT}Tất cả tài khoản đã được xử lý thành công{Fore.GREEN+Style.BRIGHT}=========", end="", flush=True)
        sys.exit()
...
def parse_arguments():
    ...
    #     task_input = input("Tự động hoàn thành nhiệm vụ? (y/n, default n): ").strip().lower()
    #     args.task = task_input if task_input in ['y', 'n'] else 'n'
    
    # if args.multi is None:
    #     multi_input = input("Bạn có muốn nâng cấp boots không? (y/n, default n): ").strip().lower()
    #     args.multi = multi_input if multi_input in ['y', 'n'] else 'n'
    
    # if args.fill is None:
    #     fill_input = input("Bạn có muốn nâng cấp fill rate? (y/n, default n): ").strip().lower()
